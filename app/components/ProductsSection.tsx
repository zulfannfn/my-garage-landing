  ...
  return (
    <section id="products" className="relative py-28 overflow-hidden">
      ...
      <div className="relative z-10 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        >
          <p className="text-red-500 text-sm font-semibold uppercase tracking-widest mb-4">{t.products.tag}</p>
          <h2 className="section-heading mb-5" style={{ color: 'var(--fg)' }}>
            {t.products.h}<br /><span className="text-gradient-red">{t.products.hRed}</span>
          </h2>
          <p className="text-base max-w-xl mx-auto" style={{ color: 'var(--fg2)' }}>{t.products.sub}</p>
        </motion.div>
            ...
            <div className="flex items-start justify-between mb-6">
              <div>
                <div className="inline-flex items-center gap-1.5 rounded-full px-3 py-1 mb-3 border"
                  ...
                  <span className="text-green-500 text-xs font-medium">{t.products.liteBadge}</span>
                </div>
                <h3 className="font-bold text-2xl" style={{ color: 'var(--fg)' }}>{t.products.liteTitle}</h3>
                <p className="text-sm mt-1" style={{ color: 'var(--muted)' }}>{t.products.liteSub}</p>
              </div>
            </div>

            <div className="mb-6">
              <p className="text-4xl font-bold" style={{ color: 'var(--fg)' }}>{t.products.liteFree}</p>
              <p className="text-sm mt-1" style={{ color: 'var(--muted)' }}>{t.products.liteFreeSub}</p>
            </div>

            <ul className="space-y-3 mb-8 flex-1">
              {t.products.liteFeatures.map((f, i) => {
            ...
            <div className="flex items-start justify-between mb-6 relative z-10">
              <div>
                <div className="inline-flex items-center gap-1.5 rounded-full px-3 py-1 mb-3 border"
                  ...
                  <span className="text-red-500 text-xs font-medium">{t.products.proBadge}</span>
                </div>
                <h3 className="font-bold text-2xl" style={{ color: 'var(--fg)' }}>{t.products.proTitle}</h3>
                <p className="text-sm mt-1" style={{ color: 'var(--muted)' }}>{t.products.proSub}</p>
              </div>
            </div>

            <div className="mb-6 relative z-10">
              <p className="text-4xl font-bold" style={{ color: 'var(--fg)' }}>{t.products.proBeta}</p>
              <p className="text-sm mt-1" style={{ color: 'var(--muted)' }}>{t.products.proBetaSub}</p>
            </div>

            <ul className="space-y-3 mb-8 flex-1 relative z-10">
              {t.products.proFeatures.map((f, i) => {
